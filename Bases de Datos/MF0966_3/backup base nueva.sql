...
-- Schema mf0966_3
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `mf0966_3` DEFAULT CHARACTER SET utf8 COLLATE utf8_bin ;
USE `mf0966_3` ;
...
CREATE TABLE IF NOT EXISTS `mf0966_3`.`alumno` (
  ...
  `direccion` VARCHAR(250) CHARACTER SET 'utf8' COLLATE 'utf8_bin' NULL DEFAULT NULL,
  `poblacion` VARCHAR(150) CHARACTER SET 'utf8' COLLATE 'utf8_bin' NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `mf0966_3`.`curso` (
  ...
  `nHoras` INT NOT NULL,
  `temario` TEXT CHARACTER SET 'utf8' COLLATE 'utf8_bin' NULL DEFAULT NULL,
  `activo` TINYINT NULL DEFAULT '1',
...
-- -----------------------------------------------------
-- Table `mf0966_3`.`valoracion`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mf0966_3`.`valoracion` (
  `idValoracion` INT NOT NULL AUTO_INCREMENT,
  `alumno_codigo` INT NOT NULL,
  `curso_codigo` INT NOT NULL,
  `opinion` VARCHAR(400) CHARACTER SET 'utf8' COLLATE 'utf8_bin' NOT NULL,
  PRIMARY KEY (`idValoracion`),
  UNIQUE INDEX `idValoracion_UNIQUE` (`idValoracion` ASC) VISIBLE,
  INDEX `fk_Valoracion_alumno1_idx` (`alumno_codigo` ASC) VISIBLE,
  INDEX `fk_Valoracion_curso1_idx` (`curso_codigo` ASC) VISIBLE,
  CONSTRAINT `fk_Valoracion_alumno1`
    FOREIGN KEY (`alumno_codigo`)
    REFERENCES `mf0966_3`.`alumno` (`codigo`),
  CONSTRAINT `fk_Valoracion_curso1`
    FOREIGN KEY (`curso_codigo`)
    REFERENCES `mf0966_3`.`curso` (`codigo`))
ENGINE = InnoDB
AUTO_INCREMENT = 9
...
-- -----------------------------------------------------
-- procedure cursoGetByIdDetalle
-- -----------------------------------------------------

DELIMITER $$
USE `mf0966_3`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `cursoGetByIdDetalle`(IN _id INT)
BEGIN
	SELECT c.nombre, c.identificador, c.nHoras, p.nombre, p.apellidos, v.opinion FROM curso c
	JOIN profesor p ON p.codigo = c.profesor_codigo
	JOIN valoracion v ON v.curso_codigo = c.codigo
	WHERE c.codigo = _id;
END$$
...
-- -----------------------------------------------------
-- procedure valoracionCreate
-- -----------------------------------------------------

DELIMITER $$
USE `mf0966_3`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `valoracionCreate`(IN _alumno_codigo INT, IN _curso_codigo INT, IN _opinion VARCHAR(400), OUT _id INT)
BEGIN
	INSERT INTO valoracion(alumno_codigo, curso_codigo, opinion) VALUES (_alumno_codigo, _curso_codigo, _opinion);
...
-- -----------------------------------------------------
-- procedure valoracionDelete
-- -----------------------------------------------------

DELIMITER $$
USE `mf0966_3`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `valoracionDelete`(IN _id INT)
BEGIN
	DELETE FROM valoracion WHERE idValoracion = _id;
END$$

DELIMITER ;

-- -----------------------------------------------------
-- procedure valoracionGetAll
-- -----------------------------------------------------

DELIMITER $$
USE `mf0966_3`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `valoracionGetAll`()
BEGIN
...
-- -----------------------------------------------------
-- procedure valoracionUpdate
-- -----------------------------------------------------

DELIMITER $$
USE `mf0966_3`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `valoracionUpdate`(IN _alumno_codigo INT, IN _curso_codigo INT, IN _opinion VARCHAR(400), IN _id INT)
BEGIN
	UPDATE valoracion
	SET alumno_codigo = _alumno_codigo, curso_codigo = _curso_codigo, opinion = _opinion
    WHERE idValoracion = _id;
END$$
