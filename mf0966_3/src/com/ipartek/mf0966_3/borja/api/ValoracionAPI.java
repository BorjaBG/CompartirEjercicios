...
public class ValoracionAPI extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * Valoracion especifica
		 * No funcional ya que obtener por ID en Valoracion no esta implementado
		 */
	...
	protected void doPut(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//Comprobamos que en la URL haya "algo" que haga de ID
		if (path == null || path.equals("/")) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
			return;
		}
		
		//Comprobamos que ese "algo" sea un numero
		if(path.matches(URL_ID_VALIDA)) {
			int id = Integer.parseInt(path.substring(1));
			...
			response.getWriter().write(gson.toJson(valoracion));
			//Si se a modificado sin problemas sacamos codigo 201
			response.setStatus(HttpServletResponse.SC_CREATED);
			return;
		}else {
	...
	protected void doDelete(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(path.matches(URL_ID_VALIDA)) {
			...
			//Si se a eliminado sin problemas sacamos codigo 200
			response.setStatus(HttpServletResponse.SC_OK);
			return;
		}else {
			//Si no coincide con el patron sacamos codigo 400
