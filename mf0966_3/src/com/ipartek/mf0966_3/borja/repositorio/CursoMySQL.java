...
public class CursoMySQL implements Dao<Curso>{
	...
	private String sqlSelectIdentificador = "CALL cursogetByIdDetalle(?)";
	...
	private Connection getConexion(){
		try {
			...
			System.err.println("IPARTEK: Error de conexion a la base de datos: " + url + ":" + usuario + ":" + contraseña);
		...
		try(Connection con = getConexion()){
			try (CallableStatement cs = con.prepareCall(sqlSelect)) {
				try (ResultSet rs = cs.executeQuery()) {
					...
					while (rs.next()) {
						cursos.add(new Curso(rs.getString("cursoidentificador"), rs.getString("cursonombre"), rs.getInt("cursonhoras"), rs.getString("profesornombre"), rs.getString("profesorapellidos")));
					}
					return cursos;
				}
			}
	...
	@Override
	public ArrayList<Curso> obtenerPorIdentificador(String identificador) {
		try (Connection con = getConexion()) {
			try(CallableStatement cs = con.prepareCall(sqlSelectIdentificador)) {
				...
				try(ResultSet rs = cs.executeQuery()){
					ArrayList<Curso> cursos = new ArrayList<>();
						while(rs.next()) {
							cursos.add(new Curso(rs.getString("identificador"), rs.getString("nombre"), rs.getInt("nhoras"), rs.getString("nombre"),
									rs.getString("apellidos"), rs.getString("opinion")));
						}
					return cursos;
				}
			}
		} catch (SQLException e) {
			throw new RuntimeException("Error al obtener el curso: " + identificador, e);
		}
	}
